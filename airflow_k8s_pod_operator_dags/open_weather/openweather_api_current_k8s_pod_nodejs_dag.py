...
# open weather API, pulls down the data for the current weather and then writes
# that data to InfluxDB. I.e., this DAG doesn't do the ETL work, it's just
# uses to run the container that does. Note: all the env vars are the same,
# so this dag can technically be used for either the Python or NodeJS variant.
# I.e. the only difference between this and the Python version is the name
# of the container.
...
with DAG(
    dag_id='openweather_current_nodejs_ts',
    ...
    k = KubernetesPodOperator(
        ...
        name="openweather_current_pod_nodejs_ts",
        task_id="openweather_current_data_nodejs_ts",
