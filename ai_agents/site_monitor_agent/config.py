...
# Config script for site monitor. Might need to rethink this,
# given this will run on K3s, might be easier to just use a
# config map.
...
def load_watch_file(path: str | Path = "review_config.yml") -> WatchFileConfig:
    with open(path, "r", encoding="utf-8") as f:
        raw: dict[str, Any] = yaml.safe_load(f) or {}
    return WatchFileConfig(**raw)


def load_config(path: str | Path = "review_config.yml") -> AppConfig:
