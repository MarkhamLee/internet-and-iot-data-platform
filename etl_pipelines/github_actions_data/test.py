...
class GitHubDependabotTesting(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.REPO_NAME = os.environ['ACCOUNT_NAME']

        # load GitHub Token
        self.GITHUB_TOKEN = os.environ['GITHUB_TOKEN']

        self.ENDPOINT = os.environ['ACTIONS_ENDPOINT']

        self.github_utilities = GitHubUtilities()

    # End to end test
    def test_github_repo_actions(self):

        # build URL
        full_url = main.build_url(self.ENDPOINT)

        # get GitHub dependabot data
        data = main.get_repo_actions(full_url, self.GITHUB_TOKEN)

        # count alerts & get alert status code
        alert_count = main.parse_data(data)
    ...
    def test_github_api_fail(self):
        ...
        # get GitHub dependabot data
        data = main.get_repo_actions(full_url, 'fake-key')
